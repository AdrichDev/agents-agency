...
import { fmt, type InvoiceRecord } from "./types";
...
/**
 * Documento imprimible de factura. Misma estructura visual que BudgetPreview
 * (UC-3, aa-facturas-desde-presupuestos-aceptados): SOLO cambia la terminología
 * visible — PRESUPUESTO → FACTURA, Fecha presupuesto → Fecha factura, número
 * → "FAC - ...". Los datos (líneas, totales, snapshots) vienen del presupuesto
 * origen (b.budget), nunca recalculados en cliente.
 */
export function InvoicePreview({ invoice: inv, onBack }: InvoicePreviewProps) {
  const b = inv.budget;
  const dateStr = new Date(inv.createdAt).toLocaleDateString("es-ES", { year: "numeric", month: "long", day: "numeric" });
  const vatImpl = b.subtotalImpl * 0.21;
  const vatMaint = b.subtotalMaint * 0.21;

  const vencimientoDate = new Date(inv.createdAt);
    ...
    <div className="w-full">
      ...
      <div className="no-print flex items-center justify-between bg-white/5 border border-edge p-4 rounded-xl mb-6">
        <div className="flex items-center gap-4">
          ...
          <Badge variant={inv.status} className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">
            {inv.status}
          </Badge>
        </div>
      ...
      <div className="print-area bg-white text-slate-900 rounded-2xl shadow-xl overflow-hidden max-w-4xl mx-auto" style={{ fontFamily: "'Inter', sans-serif" }}>
        <div className="p-10 md:p-14">
          ...
          <div className="flex justify-between items-start pb-8 mb-8" style={{ borderBottom: "2px solid #e2e8f0" }}>
            ...
            {/* Izquierda: Factura y Datos */}
            <div>
              <h2 style={{ fontFamily: "Georgia, serif", fontSize: 36, fontWeight: 900, color: "#0f172a", textTransform: "uppercase", letterSpacing: 2, margin: "0 0 16px" }}>Factura</h2>
              <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.6 }}>
                <p style={{ margin: 0 }}><strong>Fecha de Factura:</strong> {dateStr}</p>
                <p style={{ margin: 0 }}><strong>Nº:</strong> {inv.number}</p>
          ...
          <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: 24, fontSize: 11, color: "#64748b" }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 16, lineHeight: 1.7 }}>
              <li>Esta factura tiene una validez de 30 días naturales desde la fecha de emisión.</li>
