...
export default function SidebarNavItem({
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-2.5 px-3 py-1 rounded-lg text-sm leading-tight transition relative ${
    ...
    >
      {active && <div className="absolute left-0 top-0 bottom-0 w-1 bg-accent-gradient rounded-l-lg" />}
      <span className={`relative text-sm ${active ? "text-neon-cyan" : "text-slate-500"}`}>
        {icon}
        {showBadge && collapsed && (
