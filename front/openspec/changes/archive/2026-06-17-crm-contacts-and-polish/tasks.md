...
> Bloques A-E: COMPLETOS y verificados (2026-06-17). Pendiente: sdd-archive.
...
- [x] D1.4 Crear `back/prisma/migrate-crm-contacts.sql` (idempotente: enums,
       ALTER TABLE Client, backfill codCliente, CREATE TABLE ProspectContact).
- [x] D1.5 BD ya migrada (db push previo) — verificado read-only: tabla
       `ProspectContact` (22 filas), `Client.codCliente`+`direccion`, enums
       presentes, 0 codCliente null. SQL queda como artefacto idempotente reproducible.
- [x] D1.6 Añadir constraint `@unique` en `Client.codCliente` tras backfill
       (incluido en el SQL — `Client_codCliente_key`).

### D2. Router `/api/contacts`

- [x] D2.1 Crear `back/src/routes/contacts.ts` con Express `Router`.
- [x] D2.2 Implementar `GET /api/contacts` (lista paginable).
- [x] D2.3 Implementar `POST /api/contacts` con validación Zod
       (type, name, contactado enum).
- [x] D2.4 Implementar `PATCH /api/contacts/:id`.
- [x] D2.5 Implementar `DELETE /api/contacts/:id` (devuelve 204).
- [x] D2.6 Implementar `GET /api/contacts/pending-count` →
       `{ count: number }` (contactado != "si").
- [x] D2.7 Montar `contactsRouter` en `back/src/index.ts`.

### D3. API clientes enriquecida

- [x] D3.1 Añadir `codCliente` y `direccion` al select de `GET /api/clients`.
- [x] D3.2 Calcular `hasInvoices` vía `_count.budgets > 0` e incluirlo en
       la respuesta.

### D4. Automatización de leads

- [x] D4.1 En el handler que crea un `Lead` (landing/chat), añadir creación
       automática de `ProspectContact(type="lead")` en el mismo bloque
       transaccional (o inmediatamente después).
- [x] D4.2 Implementar función `notifyAdminNewLead(contact)` que usa la
       integración Gmail OAuth existente para enviar email al admin.
- [x] D4.3 Invocar `notifyAdminNewLead` en fire-and-forget (`catch` con
       `console.error`); no bloquea la respuesta HTTP.
- [x] D4.4 Leer `SystemConfig.adminEmail`; si es null/vacío, hacer fallback
       al email del primer usuario con rol admin; si no hay ninguno, omitir.

### D5. Tests bloque D

- [x] D5.1 Test `pending-count`: solo cuenta `contactado = "no"` y `"nc"`,
       no `"si"`.
- [x] D5.2 Test `notifyAdminNewLead`: fallo de Gmail no lanza excepción al
       caller (mock que rechaza).
- [x] D5.3 Test CRUD básico: POST crea con código secuencial; PATCH actualiza
...
- [x] E1.1 Añadir columnas codCliente y Dirección a la tabla de
       `front/app/clientes/page.tsx`.
- [x] E1.2 Añadir columna Facturas con icono SVG documento (rojo/verde según
       `hasInvoices`).
- [x] E1.3 El icono Facturas es enlace a `/facturacion?clientId={id}`.
- [x] E1.4 Si `/facturacion` no acepta aún `?clientId=`, añadir el filtro.

### E2. Página posibles contactos

- [x] E2.1 Crear `front/app/contactos/page.tsx` con tabla de `ProspectContact`.
- [x] E2.2 Columna Contactado: badges verde/rojo/naranja (sí/no/nc).
- [x] E2.3 Badge "N" amarillo en columna Nombre para entradas del mismo día
       con `contactado != "si"`.
- [x] E2.4 Columna Tipo: "Lead" / "Prospecto".
- [x] E2.5 Columna Fecha/Hora alta: formato "dd/mm/aaaa hh:mm".
- [x] E2.6 Acción inline para actualizar `contactado` (select o botones).

### E3. Badge de pendientes en nav

- [x] E3.1 Fetch `GET /api/contacts/pending-count` en el componente nav
       (en `useEffect([pathname])`).
- [x] E3.2 Mostrar badge amarillo con el número; ocultar si count = 0.
- [x] E3.3 Añadir enlace en el nav hacia `/contactos`.

### E4. Favicon persistente

- [x] E4.1 Auditar flujo en `ThemeInitializer.tsx`: identificar dónde se
       sobreescribe localStorage con valor `null` de DB.
- [x] E4.2 Corregir precedencia: localStorage > DB > default.
- [x] E4.3 Solo escribir en localStorage cuando DB tiene valor no nulo.
- [x] E4.4 El favicon por defecto es `/assets/3A_sin_fondo.png` si no hay
...
- [x] Z1 Ejecutar `cd back && npm test` — todos los tests verdes incluyendo
       nuevos de bloque D.
- [x] Z2 Ejecutar `cd front && npm run typecheck` — sin errores de tipos.
- [x] Z3 Verificar `next build` front sin warnings de tamaño ni errores.
- [x] Z4 Comprobar que test de regresión P7 (`/api/stats` sin params) sigue verde.
- [x] Z5 Actualizar `front/openspec/changes/README.md` con la entrada P10.
- [x] Z6 Actualizar `front/openspec/project.md`: añadir P10 en Completed Roadmap
