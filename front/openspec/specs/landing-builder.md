...
- **Phase**: P6 (Landing Builder) ✅ Completed and verified
- **Date archived**: 2026-06-12
- **Tests**: 182 passing (back), typecheck clean, front build clean
- **Technical Debt**: Playwright E2E for full builder flow (deferred)

## Known Limitations

1. **Playwright E2E** — conversational flow + generation not covered by automated tests; requires manual validation or deferred Playwright work
2. **File size limit** — 300KB max for `files Json`; large projects may truncate; documented in UI warnings
3. **Monaco bundle** — loaded dynamically; impact on initial page load not optimized
4. **Chatbot detection in future versions** — P9 adds detection; P6 has no website analysis

## Notes for Future Phases

- Landing builder domain is fully isolated and can be extended independently
- Mobile scaffold generation (Expo/Flutter) provides foundation for deeper mobile-first features
- Database provider integration (Firebase/Supabase/local-postgres) sets pattern for future integrations
