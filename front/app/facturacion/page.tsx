...
import { SERVICES_CATALOG, type BudgetRecord, type BudgetStatus } from "@/components/facturacion/types";
...
function BillingAndBudgets() {
  ...
  // Datos remotos
  const { data: budgetsData, loading, refetch } = useResource<BudgetRecord[]>("/api/budgets");
  ...
  const startNewBudget = () => {
    setDraft(emptyDraft(`AD-${new Date().getFullYear()}-${String(budgets.length + 1).padStart(3, "0")}`));
    setViewState("create");
  };

  const handleGenerate = async (payload: BudgetPayload, _linkedClientId: string) => {
    setSaving(true);
    try {
