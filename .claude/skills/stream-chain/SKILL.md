...
```bash
agentic-runtime stream-chain run \
...
```bash
agentic-runtime stream-chain pipeline analysis
```
...
```bash
agentic-runtime stream-chain run <prompt1> <prompt2> [...] [options]
```
...
```bash
agentic-runtime stream-chain pipeline <type> [options]
```
...
```bash
agentic-runtime stream-chain pipeline refactor
```
...
```bash
agentic-runtime stream-chain pipeline test
```
...
```bash
agentic-runtime stream-chain pipeline optimize
```
...
```bash
agentic-runtime stream-chain pipeline refactor --timeout 60 --verbose
```

#### Debug Test Generation

```bash
agentic-runtime stream-chain pipeline test --debug
```

#### Comprehensive Optimization

```bash
agentic-runtime stream-chain pipeline optimize --timeout 90 --verbose
```
...
Define reusable pipelines in `.agentic-runtime/config.json`:
...
```bash
agentic-runtime stream-chain pipeline security
agentic-runtime stream-chain pipeline documentation
```
...
agentic-runtime swarm init --topology mesh
...
# Results stored in .agentic-runtime/memory/stream-chain/
```
...
agentic-runtime stream-chain pipeline optimize --debug
...
```bash
agentic-runtime stream-chain run "complex task" --timeout 120
```
...
```bash
agentic-runtime stream-chain run "step 1" "step 2" --debug
```
...
cat .agentic-runtime/config.json | grep -A 10 "streamChain"
```
