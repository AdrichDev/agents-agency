...
```bash
# Check status
agentic-runtime pair --status

# View history
agentic-runtime pair --history
...
# End session
agentic-runtime pair --end [--save] [--report]
```
...
```bash
agentic-runtime pair --start --mode driver
```
...
```bash
agentic-runtime pair --start --mode navigator
```
...
agentic-runtime pair --start --mode switch

# 5-minute intervals (rapid)
agentic-runtime pair --start --mode switch --interval 5m

# 15-minute intervals (deep focus)
agentic-runtime pair --start --mode switch --interval 15m
```
...
#### Basic Configuration
Create `.agentic-runtime/pair-config.json`:
...
```bash
# Set configuration
agentic-runtime pair config set defaultMode switch
agentic-runtime pair config set verification.threshold 0.98

# Get configuration
agentic-runtime pair config get
agentic-runtime pair config get defaultMode

# Export/Import
agentic-runtime pair config export > config.json
agentic-runtime pair config import config.json

# Reset
agentic-runtime pair config reset
```
...
```bash
# Create profile
agentic-runtime pair profile create refactoring \
  --mode driver \
  --verify true \
  --threshold 0.98 \
  --focus refactor
...
# Use profile
agentic-runtime pair --start --profile refactoring

# List profiles
agentic-runtime pair profile list
```
...
agentic-runtime pair --template refactor
...
# Feature template
agentic-runtime pair --template feature
...
# Debug template
agentic-runtime pair --template debug
...
# Learning template
agentic-runtime pair --template learn
# Mode: Mentor
# Pace: Slow
# Explanations: Detailed
# Examples: Many
...
```bash
# Save session
agentic-runtime pair --save [--name <name>]

# Load session
agentic-runtime pair --load <session-id>

# Export session
agentic-runtime pair --export <session-id> [--format json|md]

# Generate report
agentic-runtime pair --report <session-id>
```
...
```bash
# Start in background
agentic-runtime pair --start --background

# Monitor background session
agentic-runtime pair --monitor

# Attach to background session
agentic-runtime pair --attach <session-id>

# End background session
agentic-runtime pair --end <session-id>
```
...
agentic-runtime pair --start --record

# Replay session
agentic-runtime pair --replay <session-id>

# Session analytics
agentic-runtime pair --analytics <session-id>
```
...
**With Git:**
```bash
agentic-runtime pair --start --git --auto-commit
```

**With CI/CD:**
```bash
agentic-runtime pair --start --ci --non-interactive
```

**With IDE:**
```bash
agentic-runtime pair --start --ide vscode
```
...
- Run `agentic-runtime pair config validate`
...
- `agentic-runtime pair --help` - Show help
- `agentic-runtime pair config` - Manage configuration
- `agentic-runtime pair profile` - Manage profiles
- `agentic-runtime pair templates` - List templates
- `agentic-runtime pair agents` - List available agents
