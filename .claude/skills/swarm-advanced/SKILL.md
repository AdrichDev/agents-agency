...
npm install -g agentic-runtime@alpha

# Add MCP server (if using MCP tools)
claude mcp add agentic-runtime npx agentic-runtime@alpha mcp start
```

### Basic Pattern
```javascript
// 1. Initialize swarm topology
mcp__agentic_runtime__swarm_init({ topology: "mesh", maxAgents: 6 })

// 2. Spawn specialized agents
mcp__agentic_runtime__agent_spawn({ type: "researcher", name: "Agent 1" })

// 3. Orchestrate tasks
mcp__agentic_runtime__task_orchestrate({ task: "...", strategy: "parallel" })
```
...
mcp__agentic_runtime__parallel_execute({
  "tasks": [
    {
      "id": "web-search",
...
mcp__agentic_runtime__pattern_recognize({
  "data": researchData,
  "patterns": ["trend", "correlation", "outlier", "emerging-pattern"]
})

// Cognitive analysis
mcp__agentic_runtime__cognitive_analyze({
  "behavior": "research-synthesis"
})

// Quality assessment
mcp__agentic_runtime__quality_assess({
  "target": "research-sources",
  "criteria": ["credibility", "relevance", "recency", "authority"]
})

// Cross-reference validation
mcp__agentic_runtime__neural_patterns({
  "action": "analyze",
...
mcp__agentic_runtime__memory_search({
  "pattern": "topic X",
  "namespace": "research",
  "limit": 20
})
...
mcp__agentic_runtime__workflow_execute({
...
npx agentic-runtime swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
mcp__agentic_runtime__batch_process({
  "items": [
...
// CI/CD pipeline
mcp__agentic_runtime__pipeline_create({
  "config": {
...
npx agentic-runtime swarm "build REST API with authentication" \
...
mcp__agentic_runtime__benchmark_run({
...
// Security scanning
mcp__agentic_runtime__security_scan({
  "target": "application",
  "depth": "comprehensive"
})

// Vulnerability analysis
mcp__agentic_runtime__error_analysis({
...
mcp__agentic_runtime__performance_report({
  "format": "detailed",
  "timeframe": "current-run"
})

// Get test results
mcp__agentic_runtime__task_results({
  "taskId": "test-execution-001"
})

// Trend analysis
mcp__agentic_runtime__trend_analysis({
...
```bash
# Quick testing swarm
npx agentic-runtime swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
// Cost analysis
mcp__agentic_runtime__cost_analysis({
  "timeframe": "30d"
})
```
...
mcp__agentic_runtime__daa_fault_tolerance({
  "agentId": "all",
...
try {
  await mcp__agentic_runtime__task_orchestrate({
  ...
  // Check swarm health
  const status = await mcp__agentic_runtime__swarm_status({})

  // Analyze error patterns
  await mcp__agentic_runtime__error_analysis({
    "logs": [error.message]
  })
...
mcp__agentic_runtime__memory_namespace({
  "namespace": "research-swarm",
  "action": "create"
})

// Create state snapshot
mcp__agentic_runtime__state_snapshot({
...
mcp__agentic_runtime__memory_backup({
...
mcp__agentic_runtime__learning_adapt({
  "experience": {
...
mcp__agentic_runtime__automation_setup({
  "rules": [
    {
...
mcp__agentic_runtime__trigger_setup({
...
mcp__agentic_runtime__topology_optimize({
  "swarmId": "current-swarm"
})

// Load balancing
mcp__agentic_runtime__load_balance({
  "swarmId": "development-swarm",
  "tasks": taskQueue
})

// Agent coordination sync
mcp__agentic_runtime__coordination_sync({
  "swarmId": "development-swarm"
})

// Auto-scaling
mcp__agentic_runtime__swarm_scale({
  "swarmId": "development-swarm",
  "targetSize": 12
})
```
...
mcp__agentic_runtime__metrics_collect({
  "components": ["agents", "tasks", "memory", "performance"]
})

// Health monitoring
mcp__agentic_runtime__health_check({
  "components": ["swarm", "agents", "neural", "memory"]
})

// Usage statistics
mcp__agentic_runtime__usage_stats({
...
mcp__agentic_runtime__swarm_init({ topology: "hierarchical", maxAgents: 8 })
...
mcp__agentic_runtime__swarm_init({ topology: "star", maxAgents: 5 })
...
mcp__agentic_runtime__swarm_init({ topology: "mesh", maxAgents: 4 })
...
- [Claude Flow Documentation](https://github.com/ruvnet/agentic-runtime)
- [Swarm Orchestration Guide](https://github.com/ruvnet/agentic-runtime/wiki/swarm)
- [MCP Tools Reference](https://github.com/ruvnet/agentic-runtime/wiki/mcp)
- [Performance Optimization](https://github.com/ruvnet/agentic-runtime/wiki/performance)
