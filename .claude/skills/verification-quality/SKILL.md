...
> **Shipped vs. aspirational.** The *concrete, in-CI* verification stack — the 6 regression-guard jobs + the witness manifest + the tool-discoverability audit — is real and runs on every push. The truth-scoring / auto-rollback / WebSocket-dashboard surface described later in this doc is partly shipped (`agenticRuntime verify` runs the witness checks) and partly design — treat the "CI Guards" section below as the authoritative current state.

## CI Guards — what's actually shipped (current state)

AgenticRuntime's regression protection is three layers, all gated before publish. Authoritative reference: [`verification/README.md`](../../../verification/README.md).
...
| **2 — cryptographic witness** | Every documented fix's load-bearing marker must still be present in dist; Ed25519-signed, per-OS bundles | `witness-verify` (ubuntu/macos/windows) — `plugins/agenticRuntime-core/scripts/witness/verify.mjs` against `verification/<os>/manifest.md.json` | ADR-103 |
...
node plugins/agenticRuntime-core/scripts/test-hooks.mjs "node $PWD/v3/@agentic-runtime/cli/bin/cli.js"
node plugins/agenticRuntime-core/scripts/test-mcp-protocol.mjs
node plugins/agenticRuntime-core/scripts/test-memory-import.mjs
node plugins/agenticRuntime-core/scripts/test-mcp-roundtrips.mjs        # #1889 paired-tool round-trip
node plugins/agenticRuntime-core/scripts/test-cli-no-crash.mjs          # #1863 unhandled-exception class
node plugins/agenticRuntime-core/scripts/test-consensus-transport.mjs   # ADR-095 G2 consensus transport

# Witness manifest — regenerate + verify
node scripts/regen-witness.mjs
node plugins/agenticRuntime-core/scripts/witness/verify.mjs --manifest verification/macos/manifest.md.json

# Temporal history
node plugins/agenticRuntime-core/scripts/witness/history.mjs --history verification/macos/history.jsonl summary
node plugins/agenticRuntime-core/scripts/witness/history.mjs --history verification/macos/history.jsonl regressions
```

### Adding a new guard

1. **Behavioral smoke** → write `plugins/agenticRuntime-core/scripts/test-<name>.mjs`. Pattern: static dist-scan first (fast, always completes), behavioral probe second with an internal timeout + a process-level watchdog so CI never hangs. Add a step to the relevant job in `v3-ci.yml`.
...
- Agentic Runtime installed (`npx agenticRuntime@alpha`)
...
npx agenticRuntime@alpha truth

# Run verification check
npx agenticRuntime@alpha verify check

# Verify specific file with custom threshold
npx agenticRuntime@alpha verify check --file src/app.js --threshold 0.98

# Rollback last failed verification
npx agenticRuntime@alpha verify rollback --last-good
```
...
npx agenticRuntime@alpha truth --period 7d

# View scores for specific agent
npx agenticRuntime@alpha truth --agent coder --period 24h

# Find files/tasks below threshold
npx agenticRuntime@alpha truth --threshold 0.8
```

**Output Formats:**
```bash
# Table format (default)
npx agenticRuntime@alpha truth --format table

# JSON for programmatic access
npx agenticRuntime@alpha truth --format json

# CSV for spreadsheet analysis
npx agenticRuntime@alpha truth --format csv

# HTML report with visualizations
npx agenticRuntime@alpha truth --format html --export report.html
```

**Real-time Monitoring:**
```bash
# Watch mode with live updates
npx agenticRuntime@alpha truth --watch

# Export metrics automatically
npx agenticRuntime@alpha truth --export .agentic-runtime/metrics/truth-$(date +%Y%m%d).json
```
...
```bash
# Verify single file
npx agenticRuntime@alpha verify check --file src/app.js

# Verify directory recursively
npx agenticRuntime@alpha verify check --directory src/

# Verify with auto-fix enabled
npx agenticRuntime@alpha verify check --file src/utils.js --auto-fix
...
npx agenticRuntime@alpha verify check --task task-123

# Verify with custom threshold
npx agenticRuntime@alpha verify check --task task-456 --threshold 0.99

# Verbose output for debugging
npx agenticRuntime@alpha verify check --task task-789 --verbose
```

**Batch Verification:**
```bash
# Verify multiple files in parallel
npx agenticRuntime@alpha verify batch --files "*.js" --parallel

# Verify with pattern matching
npx agenticRuntime@alpha verify batch --pattern "src/**/*.ts"

# Integration test suite
npx agenticRuntime@alpha verify integration --test-suite full
```
...
npx agenticRuntime@alpha verify check --json > verification.json
...
npx agenticRuntime@alpha verify rollback --to-commit abc123

# Interactive rollback with preview
npx agenticRuntime@alpha verify rollback --interactive
```

**Smart Rollback:**
```bash
# Rollback only failed files (preserve good changes)
npx agenticRuntime@alpha verify rollback --selective

# Rollback with automatic backup
npx agenticRuntime@alpha verify rollback --backup-first

# Dry-run mode (preview without executing)
npx agenticRuntime@alpha verify rollback --dry-run
```
...
```bash
# JSON report
npx agenticRuntime@alpha verify report --format json

# HTML report with charts
npx agenticRuntime@alpha verify report --export metrics.html --format html

# CSV for data analysis
npx agenticRuntime@alpha verify report --format csv --export metrics.csv

# Markdown summary
npx agenticRuntime@alpha verify report --format markdown
```

**Time-based Reports:**
```bash
# Last 24 hours
npx agenticRuntime@alpha verify report --period 24h

# Last 7 days
npx agenticRuntime@alpha verify report --period 7d

# Last 30 days with trends
npx agenticRuntime@alpha verify report --period 30d --include-trends

# Custom date range
npx agenticRuntime@alpha verify report --from 2025-01-01 --to 2025-01-31
```
...
npx agenticRuntime@alpha verify dashboard

# Custom port
npx agenticRuntime@alpha verify dashboard --port 8080

# Export dashboard data
npx agenticRuntime@alpha verify dashboard --export

# Dashboard with auto-refresh
npx agenticRuntime@alpha verify dashboard --refresh 5s
```
...
Set verification preferences in `.agentic-runtime/config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".agentic-runtime/metrics/truth-daily.json"
    }
  }
}
```
...
npx agenticRuntime@alpha verify check --threshold 0.99

# Lenient mode (90% acceptable)
npx agenticRuntime@alpha verify check --threshold 0.90

# Set default threshold
npx agenticRuntime@alpha config set verification.threshold 0.98
```
...
verify:
  stage: test
  script:
    - npx agenticRuntime@alpha verify check --threshold 0.95 --json > verification.json
    - |
      score=$(jq '.overallScore' verification.json)
...
npx agenticRuntime@alpha swarm --verify --threshold 0.98

# Hive Mind with auto-rollback
npx agenticRuntime@alpha hive-mind --verify --rollback-on-fail

# Training pipeline with verification
npx agenticRuntime@alpha train --verify --threshold 0.99
```
...
npx agenticRuntime@alpha pair --verify --real-time

# Pair with custom threshold
npx agenticRuntime@alpha pair --verify --threshold 0.97 --auto-fix
```
...
npx agenticRuntime@alpha verify watch --directory src/

# Watch with auto-fix
npx agenticRuntime@alpha verify watch --directory src/ --auto-fix

# Watch with notifications
npx agenticRuntime@alpha verify watch --notify --threshold 0.95
```
...
npx agenticRuntime@alpha truth --format json | \
  curl -X POST https://pushgateway.example.com/metrics/job/agentic-runtime \
  -d @-
...
npx agenticRuntime@alpha verify install-hook --pre-commit

# .git/hooks/pre-commit example:
#!/bin/bash
npx agenticRuntime@alpha verify check --threshold 0.95 --json > /tmp/verify.json

score=$(jq '.overallScore' /tmp/verify.json)
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "❌ Verification failed with score: $score"
  echo "Run 'npx agenticRuntime@alpha verify check --verbose' for details"
  exit 1
fi
...
npx agenticRuntime@alpha truth --verbose --threshold 0.0

# Check specific criteria
npx agenticRuntime@alpha verify check --verbose

# View agent-specific issues
npx agenticRuntime@alpha truth --agent <agent-name> --format json
```
...
npx agenticRuntime@alpha verify rollback --history
...
```bash
# Increase timeout
npx agenticRuntime@alpha verify check --timeout 60s

# Verify in batches
npx agenticRuntime@alpha verify batch --batch-size 10
```
...
- `npx agenticRuntime@alpha pair` - Collaborative development with verification
- `npx agenticRuntime@alpha train` - Training with verification feedback
- `npx agenticRuntime@alpha swarm` - Multi-agent coordination with quality checks
- `npx agenticRuntime@alpha report` - Generate comprehensive project reports
