...
**Required:**
- Claude Flow CLI installed (`npm install -g agentic-runtime@alpha`)
- Claude Code with hooks enabled
- `.claude/settings.json` with hook configurations

**Optional:**
- MCP servers configured (agentic-runtime, ruv-swarm, flow-nexus)
...
npx agentic-runtime init --hooks
```
...
npx agentic-runtime hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx agentic-runtime hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx agentic-runtime hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx agentic-runtime hook pre-edit [options]
...
Examples:
  npx agentic-runtime hook pre-edit --file "src/auth/login.js"
  npx agentic-runtime hook pre-edit -f "config/db.js" --validate-syntax
  npx agentic-runtime hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx agentic-runtime hook pre-bash --command <cmd>
...
Examples:
  npx agentic-runtime hook pre-bash -c "rm -rf /tmp/cache"
  npx agentic-runtime hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx agentic-runtime hook pre-task [options]
...
Examples:
  npx agentic-runtime hook pre-task --description "Implement user authentication"
  npx agentic-runtime hook pre-task -d "Continue API dev" --load-memory
  npx agentic-runtime hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx agentic-runtime hook pre-search --query <query>
...
Examples:
  npx agentic-runtime hook pre-search -q "authentication middleware"
```
...
```bash
npx agentic-runtime hook post-edit [options]
...
Examples:
  npx agentic-runtime hook post-edit --file "src/components/Button.jsx"
  npx agentic-runtime hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx agentic-runtime hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx agentic-runtime hook post-bash --command <cmd>
...
Examples:
  npx agentic-runtime hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx agentic-runtime hook post-task [options]
...
Examples:
  npx agentic-runtime hook post-task --task-id "auth-implementation"
  npx agentic-runtime hook post-task -t "api-refactor" --analyze-performance
  npx agentic-runtime hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx agentic-runtime hook post-search --query <query> --results <path>
...
Examples:
  npx agentic-runtime hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx agentic-runtime hook mcp-initialized --swarm-id <id>
...
```bash
npx agentic-runtime hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx agentic-runtime hook task-orchestrated --task-id <id>
...
```bash
npx agentic-runtime hook neural-trained --pattern <name>
...
```bash
npx agentic-runtime hook memory-sync --namespace <ns>
...
```bash
npx agentic-runtime hook session-start --session-id <id>
...
```bash
npx agentic-runtime hook session-restore --session-id <id>
...
Examples:
  npx agentic-runtime hook session-restore --session-id "swarm-20241019"
  npx agentic-runtime hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx agentic-runtime hook session-end [options]
...
Examples:
  npx agentic-runtime hook session-end --session-id "dev-session-2024"
  npx agentic-runtime hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx agentic-runtime hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx agentic-runtime hook notify --message <msg>
...
Examples:
  npx agentic-runtime hook notify -m "Task completed" --level info
  npx agentic-runtime hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx agentic-runtime hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx agentic-runtime hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx agentic-runtime hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx agentic-runtime hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx agentic-runtime hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx agentic-runtime hook pre-task --description "Build REST API"

// Internally calls MCP tools:
mcp__agentic_runtime__agent_spawn {
  type: "backend-dev",
  capabilities: ["api", "database", "testing"]
}

mcp__agentic_runtime__memory_usage {
  action: "store",
...
```javascript
// Hook command
npx agentic-runtime hook post-edit --file "api/auth.js"
...
mcp__agentic_runtime__neural_train {
...
```javascript
// Hook command
npx agentic-runtime hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__agentic_runtime__memory_persist {
  sessionId: "dev-2024"
}

mcp__agentic_runtime__swarm_status {
  swarmId: "current"
}
...
for FILE in $FILES; do
  ...
  npx agentic-runtime hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx agentic-runtime hook post-edit --file "$FILE" --auto-format
done
...
npx agentic-runtime hook notify \
...
# Run quality checks
npx agentic-runtime hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx agentic-runtime metrics score --format json | jq -r '.truth_score')
...
npx agentic-runtime hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx agentic-runtime hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx agentic-runtime hook post-edit \
  --file "api/auth.js" \
...
npx agentic-runtime hook post-task \
  --task-id "auth-api" \
  --analyze-performance \
  --store-decisions \
  --export-learnings
...
npx agentic-runtime hook session-restore \
...
npx agentic-runtime hook session-start --session-id "fullstack-feature"
...
# Backend work
npx agentic-runtime hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx agentic-runtime hook post-edit \
  --file "api/profile.js" \
...
npx agentic-runtime hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx agentic-runtime hook post-edit \
...
npx agentic-runtime hook session-start --session-id "debug-memory-leak"
...
npx agentic-runtime hook pre-search --query "EventEmitter"
# ... search executes ...
npx agentic-runtime hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx agentic-runtime hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx agentic-runtime hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx agentic-runtime hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx agentic-runtime hook post-edit \
    --file "$file" \
...
```bash
# Enable debug output
export AGENTIC_RUNTIME_DEBUG=true

# Test specific hook with verbose output
npx agentic-runtime hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .agentic-runtime/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx agentic-runtime hook validate-config
```
...
- Ensure agentic-runtime CLI is in PATH
...
- `npx agentic-runtime init --hooks` - Initialize hooks system
- `npx agentic-runtime hook --list` - List available hooks
- `npx agentic-runtime hook --test <hook>` - Test specific hook
- `npx agentic-runtime memory usage` - Manage memory
- `npx agentic-runtime agent spawn` - Spawn agents
- `npx agentic-runtime swarm init` - Initialize swarm
