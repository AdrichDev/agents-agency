...
import { scrapeUrl } from "@/lib/scraper/web";
...
function scrapeWithTimeout(url: string): Promise<string> {
  return Promise.race([
    scrapeUrl(url),
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error(`scrapeUrl timeout after ${SCRAPE_TIMEOUT_MS}ms`)), SCRAPE_TIMEOUT_MS)
    ),
  ]);
}
 ...
 */
export async function findCompetitors(zone: string): Promise<Competitor[]> {
...
/**
 * Scrapes each competitor's homepage and uses the LLM to extract their services.
...
async function enrichCompetitorsWithServices(competitors: Competitor[]): Promise<Competitor[]> {
  for (const comp of competitors) {
    ...
    try {
      const raw = await scrapeWithTimeout(comp.website);
      const truncated = raw.slice(0, MAX_SCRAPE_CHARS);
      ...
      const resp = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "Eres un analista de mercado. Extrae de este texto HTML una lista concisa de los servicios que ofrece la empresa (máximo 5, una frase corta cada uno). Responde SOLO con un array JSON de strings.",
          },
...
export async function buildCompetitorSection(
  ...
  let competitors = await findCompetitors(zone);

  if (competitors.length === 0) {
    return {
      key,
      title,
      markdown: `No se encontraron competidores de IA/marketing digital en la zona de **${zone}** mediante Google Places. Considera ampliar el radio de búsqueda o verificar manualmente.`,
    };
  }

  // Enrich with website services via scraping
  ...
  const rows = competitors.map((c) => {
    ...
    return `| ${c.name} | ${web} | ${rating} | ${services} |`;
  });

  const tableMarkdown = [
    "| Competidor | Web | Rating | Servicios detectados |",
    "|---|---|---|---|",
    ...rows,
  ].join("\n");
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `Eres un consultor estratégico de IA. Dados estos competidores reales de la zona, genera un análisis breve (3-5 ideas concretas) sobre cómo diferenciarse y superarlos. PROHIBIDO inventar competidores que no estén en la lista. Responde en markdown.`,
        },
        {
          role: "user",
  ...
  const markdown = `## Competidores reales en la zona\n\n${tableMarkdown}\n\n## Cómo diferenciarse y superarlos\n\n${differentiationAnalysis}`;
