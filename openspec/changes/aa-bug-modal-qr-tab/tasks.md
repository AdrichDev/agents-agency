...
- [ ] C.3 Gate humano, **con la pregunta cambiada por A.0-bis**: ya no es "comprueba que ahora funciona", sino **qué viste exactamente al reportarlo**. El caso descrito (pulsar QR con el modal abierto) no es alcanzable: el overlay tapa el botón. Hay que saber si lo que ocurrió fue otra cosa —cerrar y reabrir, otra pantalla, una versión anterior— antes de dar el change por bueno.
