...
- [x] 5.5 Conversación del OPERADOR a 3 superficies (Telegram + AA + OperaOS, estilo WhatsApp Web) — CERRADA 28/07/2026 CON ALCANCE REDUCIDO A 2 SUPERFICIES: Telegram + AA. La tercera (OperaOS/CRM) se construyó y después se retiró a propósito, ver 5.5c. Lo que queda en pie está verificado: `agents-agency/back/src/routes/operator-chat.ts` existe y desde el commit `e196fd7` su montaje exige rol `admin` (`back/src/index.ts:291`), y `front/components/telegram/TelegramWidgetGlobal.tsx` sigue montado — VIABLE NATIVO (investigación gateway 06/07/2026): `chat.history`/`chat.send {sessionKey, idempotencyKey}` sobre el RPC del gateway, sesión compartida `agent:main:main` (la misma del hilo Telegram), push por eventos WS con scope operator.read. El token del gateway NUNCA va al browser: cada web pasa por su backend.
  ...
  - [~] 5.5c **REVERTIDA POR DECISIÓN — el texto de abajo describe ficheros que YA NO EXISTEN.** El commit `creador_CRM@8f111d2` («remove Minion/Telegram widget from CRM, close operator-chat leak») retiró entero el lado CRM del espejo para cerrar una fuga: aquel `/operator-chat` quedó sin gate de staff. Verificado el 28/07/2026: `creador_CRM/back/src/routes/operator-chat.ts`, `creador_CRM/front/lib/hooks/use-operator-chat.ts` y cualquier `telegram-widget*` NO existen en el repo. Reconstruirlo reabriría la fuga; el centro de mando vive en AA. Se conserva el texto original como registro de lo que llegó a construirse:
    Cross-repo CRM: mismo proxy+widget apuntando al mismo sessionKey (el hilo ya es único en el gateway — no hay que sincronizar nada entre apps). Réplica exacta del mecanismo de AA (docker-exec sessions.json/jsonl + `POST /chat/completions`, sin RPC real). `creador_CRM/back/src/routes/operator-chat.ts` (GET /history, POST /send, montado tras authenticate+staffOnly) + `front/lib/hooks/use-operator-chat.ts` + pestaña «OpenClaw»/«CRM» en `telegram-widget.tsx`. Bug de encoding corregido (mojibake en mensaje de error). 13 tests back (`operator-chat.test.ts`, suite 571/571) + 9 tests front (`use-operator-chat.test.tsx` + `telegram-widget-operator-tab.test.tsx`, suite 700/700); tsc limpio ambos lados. Sin commitear (repo creador_CRM).
...
- [x] V2 Smoke manual: crear agente desde wizard → visible en OpenClaw (read-back) → conectar Telegram → conversación visible en AA y CRM → reply desde AA llega una vez → cita en AA visible en Google Calendar personal. — NOTA 28/07/2026: el tramo «y CRM» describe una superficie que ya no existe (ver 5.5c). El smoke fue válido cuando se ejecutó; hoy sólo son reproducibles AA y Telegram.
- [x] V3 Reviewer (sdd-verify o /code-review) antes de cada commit.

## Cierre — 28/07/2026

Cerrado con **alcance reducido a 2 superficies** (Telegram + AA) en lugar de las 3 que pedía la tarea 5.5. La tercera, el espejo en OperaOS/CRM, llegó a construirse entera (5.5c) y después se retiró a propósito en `creador_CRM@8f111d2` porque aquel `/operator-chat` del CRM quedó expuesto sin gate de staff. Reconstruirla reabriría la fuga.

Corrección importante sobre el estado anterior de este documento: 5.5c estaba marcada `[x]` describiendo tres ficheros del repo `creador_CRM` que ya no existen, y V2 sigue describiendo un smoke que incluía el CRM. Se ha comprobado fichero a fichero, no leído del documento. Lección repetida esta sesión: la prosa de un `tasks.md` es una afirmación sobre el pasado, no sobre el repo de hoy.

El lado que sí queda en pie está verificado y endurecido: `back/src/routes/operator-chat.ts` existe y desde `e196fd7` su montaje exige rol `admin`, cosa que no hacía cuando esta tarea se escribió.
