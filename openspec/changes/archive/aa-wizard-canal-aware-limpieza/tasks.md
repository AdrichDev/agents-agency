...
- [ ] **T5.2 — Verificación visual (HITL):** crear un agente (4 pasos), abrir un agente
  Telegram y confirmar que Implementación NO ofrece Widget en primer plano. (Pendiente HITL.) — ⏳ GATE HUMANO: el código ya está (`front/components/DeployPanel.tsx:579`, texto "siempre disponible. Si quieres publicarlo también en otro canal..."). Falta crear un agente de Telegram en el navegador y confirmar que el Widget no aparece en primer plano.
...
Cierre con una única acción humana pendiente (T5.2, comprobación visual del panel de despliegue con un agente de Telegram). El texto y la lógica dependiente de canal están verificados en `DeployPanel.tsx`.
