...
- [ ] `back` test suite (node:test) verde: T1–T7. — ⏳ GATE HUMANO: ejecutar la suite. Corrección al enunciado: dice "node:test" y es falso, `back/package.json:25` define `"test": "vitest run"`. El verde ya se declaró en T7.4 (786 verdes, 3 omitidos).
- [ ] `front` typecheck sin errores tras retirar SkillsStep/effort. — ⏳ GATE HUMANO: ejecutar `tsc --noEmit` en front. Los dos requisitos previos sí están: cero referencias a `SkillsStep` y `front/components/AgentModelPanel.tsx:9` documenta que no hay selector de `reasoning_effort`.
- [x] Retrocompat confirmada: agente en prod con `orderStatusUrl` sigue
  consultando pedidos. — verificado: `back/src/lib/agent/executor.ts:87` (`if (!cfg?.orderStatusUrl) {`) y `:95` (`fetchOrderStatus(...)`), con la nota de las líneas 77-78; cubierto por `back/tests/agent-backend-tools.test.ts`
- [x] `prisma migrate status` OK; migración aplicada (código mergeado ≠
  funcionando). — verificado: `20260716000000_agent_data_backend` es una de las 14 migraciones aplicadas; `migrate status` en producción responde "Database schema is up to date!"
- [ ] Paso manual del wizard (paso "Datos del negocio" obligatorio, salida
  ...
  Skills/Logs/Leads). — ⏳ GATE HUMANO: recorrido visual. Código presente en `front/app/agents/new/page.tsx:26` (paso "Datos del negocio") y `front/app/agents/[id]/page.tsx:26` (pestaña `datos`). Aviso al comprobar: la pestaña Skills VOLVIÓ a propósito con el cambio `aa-agent-skills-install-execute`, así que verla no es un defecto.
- [x] Scoping por agente verificado en el import n8n (sin cruce de tenants). — verificado: `back/tests/automations-n8n-import.test.ts:155` (409 si el workflow ya está vinculado a otro agente) y `:164` (403 si el nombre lleva el tag de otro agente)
- [ ] `sdd-verify` / `/code-review` antes de commit. — ⏳ GATE HUMANO: lanzar `sdd-verify` o `/code-review` antes del commit, según la convención del repositorio

## Cierre — 28/07/2026

Cierre con tres verificaciones marcadas con prueba en código y cuatro acciones humanas pendientes, todas de ejecución de suites o recorrido visual. Se corrige un error del propio documento: la suite de back es vitest, no `node:test`.
