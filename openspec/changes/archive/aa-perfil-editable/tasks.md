...
- [ ] V.1 Editar nombre/apellido/teléfono persiste. (requiere DB real — manual) — ⏳ GATE HUMANO: el propio documento lo marca "requiere DB real — manual": comprobar que editar nombre, apellido y teléfono persiste contra base de datos real
- [ ] V.2 Cambio de contraseña exige la antigua correcta. (requiere DB real — manual) — ⏳ GATE HUMANO: el propio documento lo marca "requiere DB real — manual": comprobar que el cambio de contraseña exige la antigua correcta
- [ ] V.3 "No recuerdo" envía reset. (requiere Supabase SMTP — manual) — ⏳ GATE HUMANO: el propio documento lo marca "requiere Supabase SMTP — manual": comprobar que "No recuerdo mi contraseña" envía el correo de restablecimiento real

## Tras verde: gate Agentic Runtime antes de commit.

## Cierre — 28/07/2026

Cierre con tres comprobaciones manuales pendientes que el propio documento ya declaraba como manuales (base de datos real y SMTP de Supabase). No queda código por escribir.
