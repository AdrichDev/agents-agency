...
- [ ] **V6** — post-deploy: un tenant en `platform` responde igual que antes; el mismo tenant
      ...
      **BLOQUEADO tras el despliegue de `25299eb` (27/07/2026), y no por un fallo.** Los tres
      ...
      Los 11 tenants con agente están en `platform` con saldo 10M, salvo JorjotasBarber a 0. — ⏳ GATE HUMANO: publicar al menos un agente. Mientras estén todos en borrador, el ciclo de vida devuelve 403 antes de que el registro de consumo llegue a mirar el modo de credencial, así que la verificación no puede ejecutarse.
...
Cierre con una única acción humana pendiente (V6: publicar un agente para poder verificar el modo de credencial de punta a punta). El propio documento ya declaraba que no necesita código.
