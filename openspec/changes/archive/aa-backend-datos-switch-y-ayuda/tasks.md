...
- [x] **T3.1 — Typecheck + suite** (`back` vitest+tsc, `front` tsc) verde. — verificado: Engram #989 (20/20 tests del panel, `tsc` limpio); commit a819007 empujado
- [x] **T3.2 — sec-review** ligero: el switch a managed_db no aprovisiona solo; guard de
  salida de managed_db intacto; no fuga apiKey. — verificado: `back/src/routes/agents.ts:395-397` (guard 400 al salir de `managed_db`), `:430-452` (el cambio a `managed_db` sólo fija el modo, no llama a provisión) y `:444` (`apiKey` es write-only, nunca se registra ni vuelve en la respuesta del PATCH)
- [x] **T3.3 — Engram.** — verificado: Engram #988 (architecture) y #989 (project), 18/07/2026, persisten la decisión F1/F2
...
Cierre completo: las tres casillas quedan marcadas con prueba. La regla de que la clave del cliente no sale nunca en una respuesta HTTP está verificada en `back/src/routes/agents.ts`.
