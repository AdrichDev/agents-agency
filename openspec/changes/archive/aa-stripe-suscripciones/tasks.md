...
- [ ] **T0.1** Aprobación humana del `proposal.md` y del nivel 4. — ⏳ GATE HUMANO: aprobación explícita del propietario sobre la propuesta y el nivel 4
- [ ] **T0.2** Confirmar el modelo de cobro: **por agente activo**, `Price` por unidad × `quantity`. — ⏳ GATE HUMANO: reconfirmar el criterio "por agente activo". El código ya lo asume: `back/src/lib/stripe/checkout.ts:83` (`const quantity = await countBillableAgents(tenantId)`)
...
- [x] **T0.3** ~~Decidir si la implantación se cobra por la plataforma o se factura aparte.~~
      **Resuelto el 27/07/2026: se factura con el módulo de facturas que ya existe.** La implantación
      ...
      el cobro recurrente.
- [ ] **T0.4** Crear cuenta de Stripe y obtener claves de **test**. Bloquea T2.5 en adelante para — ⏳ GATE HUMANO: crear la cuenta de Stripe y obtener las claves de prueba. Bloquea T6.5, G2, G3 y G4.
...
- [ ] **T3.4** `STRIPE_SECRET_KEY` y `STRIPE_WEBHOOK_SECRET` en `back/.env.example`. **Sólo el nombre, — ⏳ GATE HUMANO: añadir los nombres `STRIPE_SECRET_KEY` y `STRIPE_WEBHOOK_SECRET` a `back/.env.example`. Un hook del entorno impide a un agente leer o escribir ese fichero, así que tiene que hacerlo el propietario. Ambas variables sí están en el código (`back/src/lib/stripe/gateway.ts`, `back/src/routes/service-stripe.ts`).
      nunca un valor.**
...
- [x] **T6.4** `stripe-checkout-precio-servidor.test.ts`: E9, E10, E12. 19 verdes.
      **Dos defectos de spec corregidos en `validation.md` antes de escribir el test** (notas de
      ...
      · **E10 era imposible** — premisa "1 agente con `isTest = true`", pero `Agent` no tiene esa
        ...
        que **sí** factura.
- [ ] **T6.5** Smoke real con claves de test: alta → webhook real → estado en base de datos. — ⏳ GATE HUMANO: smoke real con claves de prueba (alta, webhook real, estado reflejado en base de datos). Depende de T0.4.
      **Bloqueado por T0.4.**
...
- [ ] **G2** `stripe:sync` en modo `test` primero; `live` sólo tras aprobación explícita. — ⏳ GATE HUMANO: ejecutar `npm run stripe:sync` en modo prueba. El script existe: `back/scripts/sync-stripe-catalog.ts`. Depende de T0.4.
- [ ] **G3** Registrar la URL del webhook en el panel de Stripe y guardar el — ⏳ GATE HUMANO: registrar la URL del webhook en el panel de Stripe y poner `STRIPE_WEBHOOK_SECRET` en Render
      `STRIPE_WEBHOOK_SECRET` en Render. **El secreto es distinto en test y en live.**
- [ ] **G4** `stripe:check` verde contra `live` antes de dar de alta al primer cliente real. — ⏳ GATE HUMANO: `npm run stripe:check` en verde contra el entorno real antes del primer cliente que pague
...
Cierre por bloqueo externo: el código de suscripciones está construido y sus tareas T1-T6 marcadas. Las ocho casillas que quedan abiertas dependen de una sola cosa que no está en el repositorio: no hay cuenta de Stripe. Ninguna es trabajo de programación.
