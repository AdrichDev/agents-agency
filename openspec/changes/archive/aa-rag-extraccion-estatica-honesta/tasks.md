...
- [ ] **T6.1 — Typecheck + suite** (`back` vitest + tsc, `front` tsc) verde. — ⏳ GATE HUMANO: ejecutar `vitest run` y `tsc` en back y `tsc` en front. Los ficheros de test existen (`back/tests/{web-extraction,ingest-website-status,initial-ingest-honest-status,knowledge-search}.test.ts`) y el verde se declaró en T1.2 (976 passed, 3 omitidos).
- [ ] **T6.2 — Prueba real (HITL):** re-indexar fpeuroformac.com desde la tab Conocimiento — ⏳ GATE HUMANO: reindexar fpeuroformac.com desde la pestaña Conocimiento y probar el endpoint de búsqueda contra el servicio real
  → ver chunks > 0 y estado honesto; probar el endpoint search.
- [x] **T6.3 — sec-review:** el extractor no ejecuta JS/remoto (SSRF sigue vía
  `safeFetch`); endpoint search gated; no fuga de contenido de otros tenants. — verificado: sin ejecución de JavaScript ni acceso remoto arbitrario (`back/src/lib/scraper/web.ts:3` usa `linkedom`, un DOM sin motor de JS; `:45` y `:145` pasan por `safeFetch`), y el endpoint de búsqueda está tras sesión (`back/src/routes/knowledge.ts:161`, sin presencia en `back/src/lib/public-routes.ts`). MATIZ que conviene registrar: ese handler acepta el `agentId` de params sin comprobar propiedad, así que cualquier usuario de STAFF puede leer los chunks de cualquier agente. Dentro del modelo actual el staff es del estudio, pero NO es un chequeo de tenant.
- [x] **T6.4 — Engram:** persistir el hallazgo (NO era SPA; fix por código; readability). — verificado: persistido en Engram como observación #1045 (28/07/2026), incluida la deuda del `agentId` sin comprobación de propiedad
...
Cierre con dos casillas marcadas con prueba y dos acciones humanas pendientes de ejecución. Queda registrada una deuda de seguridad: `back/src/routes/knowledge.ts:161` no comprueba que el `agentId` pertenezca a quien pregunta; hoy sólo alcanza a staff del estudio, pero no es un chequeo de tenant.
