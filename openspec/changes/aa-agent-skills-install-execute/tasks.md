...
- [ ] T3.1 [reusa] `front/app/agents/[id]/page.tsx`: añadir
...
- [ ] T3.2 [reusa] `front/components/agents/SkillsTab.tsx`: sección
...
- [ ] T3.3 [reusa] `SkillsTab.tsx`: sección "Marketplace" con buscador paginado
...
- [ ] T4.1 [reusa] `front/app/agents/new/page.tsx`: reinsertar `SkillsStep`
...
- [ ] T5.1 `cd back && npm test && npm run typecheck` (suite verde, sin
  regresiones).
- [ ] T5.2 `cd front && npm run typecheck`.
...
- [ ] T5.4 Retrocompat: con 0 `AgentSkill`, chat de un agente existente sin
  cambio observable.
- [ ] T5.5 `sdd-verify` / `/code-review` antes de commit (convención repo).
