...
- [ ] **T4.1 — Typecheck + suite** (`back` vitest+tsc, `front` tsc) verde.
- [ ] **T4.2 — E2E EN VIVO (Gru, contra prod):** vía script tsx (carga .env, importa
  ...
  agente_id, y BORRAR las filas de prueba. Confirma que el SQL cualificado resuelve.
- [ ] **T4.3 — sec-review:** aislamiento por agentId en código intacto (bindeo agente_id en
  escrituras, filtro en lecturas); se documenta la pérdida de RLS-por-rol y el follow-up
  (app.current_agent). No fuga entre agentes en la lógica.
- [ ] **T4.4 — Engram.**
